...
__Коллекция__ — объект, хранящий набор значений одного или различных типов, позволяющий обращаться к этим значениям, а также применять специальные функции и методы, зависящие от типа коллекции. Коллекции не являются итераторами сами по себе, но позволяют создать итератор на своей основе.
...
__последовательностями__ - коллекции, элементы которых пронумерованы индексами и расположены в строгом порядке. Например, списки, строки и кортежи

![image](https://github.com/user-attachments/assets/926d0795-cbb4-4a63-b496-99280c6581ab)
...
- `map(function, *iterable)` применяет пользовательскую функцию function к каждому элементу итерируемого объекта iterable. Каждый элемент iterable отправляется в функцию function в качестве аргумента.

- `filter(function, iterable)` фильтрует (отбирает) элементы переданного итерируемого объекта iterable при помощи пользовательской функции function. Если фильтрующая функция function вернёт True, то элемент из итерируемого объекта iterable попадёт в результат выполнения функции filter(), если False — не попадёт.
...
- `reversed(seq)` перебирает элементы итерируемого объекта seq в обратном порядке.

Примечания:
1. Встроенной функции next() можно передать второй аргумент, который будет возвращен вместо возбуждения исключения StopIteration, если в итераторе больше не осталось элементов.
2. Не забывайте, что коллекции не являются итераторами. Поэтому встроенная функция next() не позволяет получить очередной элемент, например списка, строки и т.д.
3. функция len() не применима к итераторам.
4. На основе одного списка (кортежа, строки, множества, словаря и т.д.) мы можем создавать множество несвязанных между собой итераторов. Каждый из них будет независимо от других перемещаться от начала до конца.
![image](https://github.com/user-attachments/assets/af8a51af-bd4a-48dc-8f59-db1d3e997df5)

# Генератор

 это итератор, который порождает значения, переданные yield
...
Функция генератор – это функция, которая возвращает итератор. Она выглядит как обычная функция, за исключением того, что использует выражение yield, а не return. Сохраняет локальные переменные

Примечания:
1. Генераторную функцию get_data() можно упростить, если использовать синтаксическую конструкцию yield from <iterable>
...
еще более простой, чем генераторные функции, содержащие оператор yield, способ создания итераторов
...
with - оператор для удобного и своевременного освобождения ресурсов. кулисами разворачивается практически тот же самый блок try-finally.
...
Примечания:
1. Атрибут closed экземпляров типа TextIOWrapper позволяет проверить состояние закрытости файла.
2. С помощью методов readable() и writable() мы можем определить возможность чтения и записи из файла.
