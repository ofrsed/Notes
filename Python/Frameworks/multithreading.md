...
- queue.Queue(maxsize=0) - создает очередь FIFO
- queue.LifoQueue(maxsize=0) - создает очередь LIFO
- queue.PriorityQueue(maxsize=0) - создает очередь с приоритетом
- SimpleQueue (не предусматривает работу с задачаами)
