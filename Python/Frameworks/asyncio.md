...
__Стек вызовов__ - это структура данных, которая используется в программировании для отслеживания порядка вызовов функций. Каждый раз, когда функция вызывается, ее контекст сохраняется в стеке, а когда функция завершается, ее контекст удаляется из стека. Это позволяет программистам отслеживать, какие функции были вызваны в каком порядке, и легче отлаживать свой код. В контексте asyncio стек вызовов отображает иерархический список функций и методов, которые вызываются в рамках определенной задачи, и может быть использован для понимания, как работает ваш асинхронный код.
...
- Статус задачи и обращение к результатам задачи:
  ...
  - task.result() — возвращает результат выполнения задачи, если она выполнена. Если задача была отменена, то метод вызывает исключение CancelledError. Если результат задачи еще не доступен, то метод вызывает исключение InvalidStateError. При ошибке цикл продолжает работу, но задача сохраняет ошибку
...
- Получение связанных с задачей объектов:
  - task.get_coro() — возвращает корутину, связанную с задачей.
    - к нему можно применить  методы .send(), .throw() и .close() Эти методы предназначены для работы с генераторами и корутинами на низком уровне.
  - task.get_loop() — возвращает цикл событий, связанный с задачей.(task.get_loop() получает его из контекста объекта Task, в то время как asyncio.get_running_loop() получает его из текущего контекста исполнения, т.е. из основного модуля asyncio.)
  - task.get_stack(*, limit=None) — возвращает стек выполнения задачи.
  - task.print_stack(*, limit=None, file=None) — выводит стек выполнения задачи в файл или на экран.
...
- Функции модуля asyncio для работы с задачами:
  - asyncio.current_task() — Возвращает текущий запущенный экземпляр Task или None, если ни одна задача не запущена.
    ...
    `current_task = asyncio.current_task()` - # Получение текущего объекта задачи asyncio.
    - `<Task pending name='Task-2' coro=<my_task() running at F:\Async\main.py:36> cb=[gather.<locals>._done_callback() at C:\Users\...\asyncio\tasks.py:754]>`
      - <Task pending — означает, что задача Task-2 имеет статус pending(ожидание) — задача не завершена.
      - `name='Task-2'` — имя задачи, которое присваивается автоматически, либо может быть задано вручную, при создании задачи. Оно не обязательно соответствует порядковому номеру задачи (например, Task-1 в нашем примере отдано под задачу, созданную из main()). При автоматическом режиме создания имени оно будет соответствовать порядку помещения в очередь ожидания цикла событий. Эти имена служат для идентификации задач внутри событийного цикла и для отладки, но они не предоставляют информацию об очередности запуска / порядке получения результатов выполнения задач в вашем коде. Имена задач присваиваются с использованием внутреннего счетчика, который увеличивается каждый раз, когда создается новый объект Task. Этот счетчик может быть общим между разными частями кода, создающими задачи.
      - `coro=<my_task() running at F:\Async\main.py:36>` — указывает, что текущая корутина связана с функцией my_task() в файле main.py на строке 36.
      - `cb=[gather.<locals>._done_callback() at C:\...\asyncio\tasks.py:754]>` — это информация о внутреннем механизме asyncio.gather(), который вызывает функцию _done_callback() после завершения каждой задачи.
        задачу можно отменить в процессе выполнения
  - `asyncio.all_tasks()` —возвращает множество (<class 'set'>) еще не завершенных объектов Task, выполняемых циклом.
      - Примеры, когда функция полезна
        -   Монмторинг и отладка
        - Управление задачами
        - Ожидание всех задач (с gether) `asyncio.gather(*asyncio.all_tasks())`
        - Ограничение количества одновременно выполняющихся задач
        - Реализация собственных стратегий обработки ошибок
