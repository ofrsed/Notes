...
# CRON
запланировать задачу по расписанию

- `crond` - служба для запуска по расписанию 
- `crontab -e` - запланировать задачу. Открывается редактов
`***** /home/max/app-test,sh >> /tmp/app-status.txt` - первые 5 символов - время (минута, час, день, месяц, день недели), 6 поле - команда, которую нужно запустить
- `crontab -l` - вывести все запланированные команды
- `tail -n -1 /var/log/syslog` - проверить что задача была выполнена
- `` - 
- `` - 

# SSH и SCP
Удаленый досуп к серверу по незащищенной сети

По умолчанию 22 порт

- `ssh <hostname ИЛИ ip-adress>` -
- `ssh <user>@<hostname ИЛИ ip-adress>` - 
- `ssh <hostname ИЛИ ip-adress>` - 

- `ssh-keygen -t rsa` - создать ключ у себя на машине
  - `/home/<user>/.ssh/id_rsa.pub` - открытый ключ
  - `/home/<user>/.ssh/id_rsa` - закрытый ключ

- `scp <имя файла> <hostname>:<куда положить>` - копировать файл по SSH


# Основа сетей


- `ip link` - интерфейсы хоста 
- `route` - таблица маршрутизации ядра 
- `ap addr` - просмотр ip адресов, назначенной этой сетью
...
- `sudo useradd <имя>` - добавить пользователя
- `sudo passwd <имя пользователя>` - установить пароль пользователю
- `whoami` - узнать пользователя
- `userdel <имя пользователя>` - удалить пользователя
- `groupadd -g 1015 devops` - создать группу
...
- след 3: group (для группы)
- след 3: other (для всех остальных)
  -`r` — read — чтение, просмотр
  -`w` — write - редактирование
  -`o` — execute - запуск


- `chmod <разрешение> <файл>` - изменить разрешение файла
