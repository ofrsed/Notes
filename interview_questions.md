...
Модульные - Проверяют работу какого-то модуля, функции
Интеграционные - проверяют интеграцию компонентов. Проверяет функции не по отдельности, а в связке (например подряд)
Сквозные - End-TO-End - тестируют полностью путь от например регистрации, до пополнения корзины и выполнения заказа
