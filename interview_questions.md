...
Cookie - это небольшой фрагмент данных, который сервер отправляет браузеру, а затем браузер включает его в каждый запрос к серверу в рамках сеанса пользователя. Это позволяет серверу "запомнить" состояние пользователя между различными запросами.

Токен авторизации — это файл определённого формата, который выдаёт пользователю автоматизированная система или сайт на определённый период — время жизни токена
